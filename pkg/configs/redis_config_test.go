...
// TestParallelRedisConfigLogic тестирует параллельное создание конфигов Redis
func TestParallelRedisConfigLogic(t *testing.T) {
	tests := []struct {
		name       string
		host       string
		port       string
		...
		{"service1", "localhost", "6379", "pass1", "0", true},
		{"service2", "redis.example.com", "6380", "pass2", "5", true},
		{"service3", "", "6379", "pass3", "0", false},           // missing host
		{"service4", "localhost", "", "pass4", "0", false},      // missing port
		{"service5", "localhost", "6379", "", "0", false},       // missing password
		{"service6", "localhost", "6379", "pass6", "16", false}, // DB out of range
		{"service7", "localhost", "6379", "pass7", "-1", false}, // DB negative
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			testEnv.Set("REDIS_CACHE_HOST", tt.host)
			testEnv.Set("REDIS_CACHE_PORT", tt.port)
			testEnv.Set("REDIS_CACHE_PASSWORD", tt.password)
			if tt.db != "" {
				testEnv.Set("REDIS_CACHE_DB", tt.db)
			}
			testEnv.Apply()
			...
			if tt.shouldWork {
				...
				// Дополнительные проверки для успешных кейсов
				if err == nil && config != nil {
					if config.Host != tt.host {
						t.Errorf("Host = %s, want %s", config.Host, tt.host)
					}
					if config.Port != tt.port {
						t.Errorf("Port = %s, want %s", config.Port, tt.port)
					}
					if config.Password != tt.password {
						t.Errorf("Password = %s, want %s", config.Password, tt.password)
					}
				}
			} else {
				if err == nil {
